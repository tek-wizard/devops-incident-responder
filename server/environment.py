...
from .models import IncidentAction, SystemObservation, SystemState
...
class DevOpsEnv:
    ...
    def reset(self, task_id: str = "service_restart"):
        ...
        self.flakiness_rate = random.uniform(*self.task["flakiness_range"])
    ...
    def _apply_flakiness(self):
        return random.random() < self.flakiness_rate
    ...
    def _complete_expected_step(self, expected_step: str):
        if expected_step == "restart_service":
            ...
            self.state.is_database_locked = False
    ...
    def _set_nominal_metrics(self):
        ...
        self.state.internal_error_rate = nominal["error_rate"]
    ...
    def _refresh_state(self):
        if self.task_id == "service_restart":
            if self.progress_index >= len(self.required_sequence):
                ...
                self.state.internal_error_rate = 0.91
                self.state.active_incidents = ["auth_api_process_crash"]
            ...
            if self.progress_index >= len(self.required_sequence):
                ...
                self.state.internal_error_rate = 0.42
                self.state.active_incidents = ["worker_memory_leak"]
            else:
                self.service_status["auth-api"] = "DOWN"
                ...
                self.state.internal_error_rate = 0.18
                self.state.active_incidents = ["service_restart_pending"]
            ...
            if self.progress_index >= len(self.required_sequence):
                ...
                self.state.internal_error_rate = 0.37
                self.state.is_database_locked = True
                self.state.active_incidents = ["db_connection_pool_exhaustion"]
            else:
                self.service_status["main-db"] = "RUNNING"
                ...
                self.state.internal_error_rate = 0.14
                self.state.is_database_locked = True
                self.state.active_incidents = ["stale_connection_pool_state"]

    def _append_telemetry_logs(self):
        for _ in range(2):
            if random.random() < self.task["noise_level"]:
                self.logs.append(random.choice(self.NOISE_LOGS))
            else:
                if self.task_id == "memory_leak":
                    self.logs.append(random.choice(self.MEDIUM_SIGNAL_LOGS))
                elif self.task_id == "db_connection_exhaustion":
                    self.logs.append(random.choice(self.HARD_SIGNAL_LOGS))

    def _is_resolved(self):
        nominal = self.task["nominal_metrics"]
        return (
            self.metrics["error_rate"] <= nominal["error_rate"] + 0.01
            and self.metrics["latency"] <= nominal["latency"] + 0.02
            and self.progress_index >= len(self.required_sequence)
        )
